...
import com.imaginea.cloudjee.utils.ConfigProperties;
...
public class RestApiTest extends BaseTest{
	...
	@Test
	@Parameters({"type","appFileName","appState","instanceGroupName","name","sslEnabled","url"})	
	public void verifyDeployOfApp(String type,String appFileName,String appState, String instanceGroupName,String name,String sslEnabled,String url) throws ParseException{
		...
		Assert.assertEquals(bodyNode.getTextValue(), "application","type doesn't match");
		bodyNode = node.findValue("appFileName");				
		Assert.assertEquals(bodyNode.getTextValue(), ConfigProperties.APP_NAME+".war","APP Name doesn't match");
		bodyNode = node.findValue("appState");				
		Assert.assertEquals(bodyNode.getTextValue(), "STARTED","type doesn't match");
		bodyNode = node.findValue("instanceGroupName");				
		Assert.assertEquals(bodyNode.getTextValue(), "defaultgroup","Default group doesn't match");
		bodyNode = node.findValue("name");				
		Assert.assertEquals(bodyNode.getTextValue(), ConfigProperties.APP_NAME,"APP Name doesn't match");
		...
		Assert.assertEquals(bodyNode.getTextValue(), "http://ashok.apps.mywavemaker.com/sample/","type doesn't match");
		Assert.assertTrue(node.findValue("time-taken").getIntValue()<10,"Time taken is more than 10");
	}
	...
	public void verifyListOfApp() throws ParseException{
		...
		Assert.assertTrue(bodyNode.toString().contains("application"),"Type doesn't match");
		bodyNode = node.findValues("appFileName");				
		Assert.assertTrue(bodyNode.toString().contains(ConfigProperties.APP_NAME+".war"),"APP Name doesn't match");
		bodyNode = node.findValues("appState");				
		Assert.assertTrue(bodyNode.toString().contains("STARTED"),"type doesn't match");
		bodyNode = node.findValues("instanceGroupName");				
		Assert.assertTrue(bodyNode.toString().contains("defaultgroup"),"Default group doesn't match");
		bodyNode = node.findValues("name");				
		Assert.assertTrue(bodyNode.toString().contains(ConfigProperties.APP_NAME),"APP Name doesn't match");
		...
		Assert.assertTrue(bodyNode.toString().contains("http://ashok.apps.mywavemaker.com/sample/"),"type doesn't match");
		Assert.assertTrue(node.findValue("time-taken").getIntValue()<10,"Time taken is more than 10");
